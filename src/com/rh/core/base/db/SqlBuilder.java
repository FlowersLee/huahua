 ...
 * 
 * @author Jerry Li
 * @version $Id: SqlBuilder.java,v 1.1 2015/12/11 05:17:43 fuxiaole Exp $
 */
        ...
        for (Object key : paramBean.keySet()) {
            ...
            if ((item != null) && (item.getInt("ITEM_TYPE") == ServConstant.ITEM_TYPE_TABLE)) {
                ...
                if (item.getStr("ITEM_FIELD_TYPE").equals(ServConstant.ITEM_FIELD_TYPE_NUM)) { //处理数据类型
                	if (paramBean.get(key) == null || "".equals(paramBean.get(key))) {
                		values.add(null);
                	} else if (item.getStr("ITEM_FIELD_LENGTH").indexOf(Constant.SEPARATOR) > 0) { //浮点
    ...
    public String insert(ServDefBean servDef, Bean dataBean, List<Object> preValue) {
        ...
        for (Bean item : items) {
            ...
            if (item.getStr("ITEM_FIELD_TYPE").equals(ServConstant.ITEM_FIELD_TYPE_NUM) 
            	...
            	if (dataBean.get(itemCode) == null || "".equals(dataBean.get(itemCode))) {
            		preValue.add(null);
            	} else if (item.getStr("ITEM_FIELD_LENGTH").indexOf(Constant.SEPARATOR) > 0) { //浮点
