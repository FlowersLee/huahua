...
public class LoginServ extends BaseServ {
    ...
    public OutBean login(ParamBean bean) {
        ...
        String loginStyle = paramBean.getStr("loginStyle");
        
        ServDefBean servDef = ServUtils.getServDef(servId);
        checkActExpression(servDef, "login", paramBean);
        // 如果用户将SY_LOGIN_MODULE_NAME设置为“custom”，SY_LOGIN_MODULE_CLASS配置会生效
        String loginModuleName = Context.getSyConf("SY_LOGIN_MODULE_NAME", "");
        String loginModuleClass = Context.getSyConf("SY_LOGIN_MODULE_CLASS", "");

        if(loginStyle != null && "ADAccount".equals(loginStyle))
        {
        	loginModuleName = "AD";
        }
    ...
    public OutBean getJianUsers(ParamBean paramBean) {
		 ...
		 * @throws SQLException 
		 */
		public OutBean initPassWord(ParamBean paramBean ) throws SQLException{
			String userCode = paramBean.getStr("USER_CODE");//登陆人员的主键
			String password = paramBean.getStr("PASSWORD");
			OutBean result = new OutBean();
			 Connection conn = null;
			    	try{
			    		conn = Context.getConn();
			    		String md5 = EncryptUtils.encrypt(password, 
				                Context.getSyConf("SY_USER_PASSWORD_ENCRYPT", EncryptUtils.DES));
						
							String sqls = new StringBuilder().append("update sy_org_user set USER_PASSWORD='"+md5+"'")
			                        .append(" where user_code='").append(userCode).append("'").toString();
							//更新
							Context.getExecutor().execute(conn, sqls.toString());
         ...
         * @return 加密后密码
         */
		public static String MD5Encryptions(String password){
