...
//删除之前校验
	...
	protected void beforeDelete(ParamBean paramBean) {
		...
		{
			String dept_code = deleteBeans.get(i).getStr("DEPT_CODE");
			Bean tempBean = new Bean().set("S_FLAG", 1).set("DEPT_CODE",dept_code);
			List<Bean> resultBeans = ServDao.finds("SY_ORG_USER", tempBean);
			if(resultBeans.size()>0)
				flag = true;
		}
		if(flag){
			 throw new TipException("部门下存在员工，不能删除！");
		}
